        ...
        this.input?.addEventListener('change', async (e) => {
            ...
            if (file) {
                try {
                    ...
                    if (this.btnUse) this.btnUse.style.display = 'inline-block';
                    if (this.btnConfirm) this.btnConfirm.style.display = 'none';
                    if (this.card) this.card.classList.remove('invalid-field');
                } catch (err) {
...
function showError(el, msg) {
    ...
    if (el.tagName === 'INPUT' && (el.type === 'text' || el.type === 'tel' || el.type === 'date' || el.type === 'email')) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Disability Logic
    const disabilityCBs = document.querySelectorAll('input[name="disability"]');
    const otherCont = document.getElementById('disability-other-container');
    const otherInp = document.getElementById('disabilityType');

    disabilityCBs.forEach(cb => {
        cb.addEventListener('change', () => {
            if (cb.value === 'None' && cb.checked) {
                disabilityCBs.forEach(c => { if (c !== cb) c.checked = false; });
                if (otherCont) otherCont.style.display = 'none';
                if (otherInp) otherInp.value = '';
            } else if (cb.checked) {
                const noneCb = document.querySelector('input[name="disability"][value="None"]');
                if (noneCb) noneCb.checked = false;
                if (cb.id === 'disabilityOtherCheck' && otherCont) otherCont.style.display = 'block';
            } else if (cb.id === 'disabilityOtherCheck' && otherCont) {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            const selectedDis = Array.from(disabilityCBs).filter(c => c.checked).map(c => c.value);
            const disGroup = document.getElementById('disabilityGroup');
            if (selectedDis.length === 0) firstErr = firstErr || showError(disGroup, "Required");
            if (selectedDis.includes('Other') && !getVal('disabilityType')) firstErr = firstErr || showError(otherInp, "Required");
            ...
            if (level && (level.value === 'Quran' || level.value === 'Kitab') && !getCheck('levelDetail')) {
                const specificGroup = level.value === 'Quran' ? document.getElementById('quranOptions') : document.getElementById('kitabOptions');
                const targetEl = specificGroup ? specificGroup.querySelector('.radio-group') : levelGroup;
                firstErr = firstErr || showError(targetEl, "Required");
            }
            ...
            try {
                ...
                const { error } = await supabaseClient.from('students').insert([{
                    ...
                    medical_condition: selectedDis.includes('Other') ? getVal('disabilityType') : selectedDis.join(','),
                    ...
                    id_photo_url: JSON.stringify({ front: idUrls.front, back: idUrls.back }),
                    receipt_url: rUrl
                }]);
    ...
    document.querySelectorAll('a[href^="#"]').forEach(anchor => {
        anchor.addEventListener('click', function (e) {
            ...
            if (target) {
                ...
                // If target not on this page, go to index.html with hash
